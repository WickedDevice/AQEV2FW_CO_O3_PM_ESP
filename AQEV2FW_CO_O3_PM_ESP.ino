...
#define EEPROM_CO_SENSITIVITY     (EEPROM_MQTT_PASSWORD - 4)      // float value, 4-bytes, the sensitivity from the sticker
...
#define EEPROM_CO_BASELINE_VOLTAGE_TABLE (EEPROM_NTP_TZ_OFFSET_HRS - (5*sizeof(baseline_voltage_t))) // array of (up to) five structures for baseline offset characterization over temperature
#define EEPROM_O3_BASELINE_VOLTAGE_TABLE (EEPROM_CO_BASELINE_VOLTAGE_TABLE - (5*sizeof(baseline_voltage_t))) // array of (up to) five structures for baseline offset characterization over temperature
#define EEPROM_MQTT_TOPIC_SUFFIX_ENABLED  (EEPROM_O3_BASELINE_VOLTAGE_TABLE - 1)    // a simple flag to indicate whether or not the topic suffix is enabled
...
#define EEPROM_CO_ZERO_NEGATIVE_RESULTS (EEPROM_DISABLE_SOFTAP - 1)
...
#define EEPROM_O3_SENSITIVITY    (EEPROM_PM10P0_CAL_OFFSET - 4)      // float value, 4-bytes, the sensitivity from the sticker  [UNUSED]
#define EEPROM_O3_CAL_SLOPE      (EEPROM_O3_SENSITIVITY - 4)     // float value, 4-bytes, the slope applied to the sensor   [UNUSED]
...
#define EEPROM_TEMPERATURE_OFFLINE_OFFSET (EEPROM_O3_ZERO_NEGATIVE_RESULTS - 4)
...
#define EEPROM_BACKUP_PM10P0_CAL_OFFSET  (EEPROM_BACKUP_O3_CAL_OFFSET + 4)
...
#define EEPROM_BACKUP_O3_SENSITIVITY     (EEPROM_BACKUP_CO_CAL_OFFSET + 4)
...
#define BACKUP_STATUS_CO_CALIBRATION_BIT          (5)
...
void initializeHardware(void) {
    ...
    Serial.println(F(" |      CO / O3 / PM Sensor Suite     |"));
...
// only apply it if the slope is not already self consistent with the sensitivity
    float sensitivity = eeprom_read_float((const float *) EEPROM_CO_SENSITIVITY);
    float calculated_slope = convert_co_sensitivity_to_slope(sensitivity);
    float stored_slope = eeprom_read_float((const float *) EEPROM_CO_CAL_SLOPE);
...
void print_eeprom_value(char * arg) {
    if (strncmp(arg, "mac", 3) == 0) {
        ...
        print_label_with_star_if_not_backed_up("O3 Sensitivity [nA/ppb]: ", BACKUP_STATUS_O3_CALIBRATION_BIT);
        print_eeprom_float((const float *) EEPROM_O3_SENSITIVITY);
        ...
        print_eeprom_float((const float *) EEPROM_O3_CAL_OFFSET);
        Serial.print(F("    "));
        Serial.print(F("O3 Report Negative values as zero: "));
        ...
        Serial.print(F("    "));
        Serial.println(F("O3 Baseline Voltage Characterization:"));
...
void restore(char * arg) {
    ...
    if (strncmp(arg, "defaults", 8) == 0) {
        ...
        eeprom_write_block("aqev2_co_o3_pm_esp", (void *) EEPROM_UPDATE_FILENAME, 32);
    }
...
// convert from nA/ppb to ppb/V
// from SPEC Sensors, Sensor Development Kit, User Manual, Rev. 1.5
// M[V/ppb] = Sensitivity[nA/ppb] * TIA_Gain[kV/A] * 10^-9[A/nA] * 10^3[V/kV] * 10^-3[ppb/ppb]
...
boolean publishHeartbeat() {
    ...
    snprintf(scratch, SCRATCH_BUFFER_SIZE-1,
             ...
             "\"converted-value\":%d,"
             "\"firmware-version\":\"%s\","
             "\"publishes\":[\"particulate\",\"o3\",\"co\",\"temperature\",\"humidity\"%s],"
             "\"counter\":%lu"
...
void printCsvDataLine() {
    ...
    if(init_o3_afe_ok && init_o3_adc_ok && (o3_ready || (sample_buffer_idx > 0))) {
        float converted_value = 0.0f, compensated_value = 0.0f;
        o3_moving_average = calculateAverage(&(sample_buffer[O3_SAMPLE_BUFFER][0]), num_samples);
        o3_convert_from_volts_to_ppb(o3_moving_average, &converted_value, &compensated_value);
        o3_ppb = compensated_value;
